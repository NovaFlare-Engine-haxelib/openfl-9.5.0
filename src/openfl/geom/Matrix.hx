...
class Matrix
{
	...
	**/
	...
	/**
		Applies the geometric transformation represented by the Matrix object to the
		specified vector of points.
		
		@param vin The vector of points to transform (x, y, x, y, ...)
		@param vout The vector to store the transformed points
	**/
	public function transformVectors(vin:Vector<Float>, vout:Vector<Float>):Void
	{
		...
		if (vinIter != null && voutIter != null)
		{
			...
			if (vinArray != null && voutArray != null)
			{
				...
				// Ensure length is multiple of 2 for safety in the loop
				var count = Std.int(len / 2) * 2;

				untyped __cpp__("
					const double* inPtr = &vinArray[0];
					double* outPtr = &voutArray[0];
					
					// Load matrix values into local variables for register reuse
					double ma = a;
					double mb = b;
					double mc = c;
					double md = d;
					...
					for(int i = 0; i < count; i+=2) {
						...
						outPtr[i]   = px * ma + py * mc + mtx;
						outPtr[i+1] = px * mb + py * md + mty;
					}
				");
				return;
			}
