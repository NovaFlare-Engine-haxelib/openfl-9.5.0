...
class Stage extends DisplayObjectContainer #if lime implements IModule #end
{
	...
	**/
	...
	@:noCompletion private function __onLimeCreateWindow(window:Window):Void
	{
		...
		window.onlyUpdate.add(__onLimeOnlyUpdate);
	...
	//仅仅负责广播ENTER_UPDATE事件
	@:noCompletion private function __onLimeOnlyUpdate():Void
	{
		__broadcastEvent(new Event(Event.ENTER_UPDATE));
	}
	...
	inline function calcScreen(stageWidth:Float, stageHeight:Float, wantWidth:Int = 1280, wantHeight:Int = 720) {
		...
		#else
		var ratioX = Std.int(Math.min(stageWidth, wantWidth) / wantWidth);
		var ratioY = Std.int(Math.min(stageHeight, wantHeight) / wantHeight);
		var ratio = Math.max(ratioX, ratioY);
		__logicalWidth = Math.ceil(Math.min(stageWidth, wantWidth) / ratio);
		__logicalHeight = Math.ceil(Math.min(stageHeight, wantHeight) / ratio);
		#end
	}
